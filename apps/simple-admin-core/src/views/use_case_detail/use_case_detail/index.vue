<script lang="ts" setup>
  ...
  import { deleteUseCaseDetail, getUseCaseDetailList } from '#/api/use_case_detail/useCaseDetail';
  import { type ActionItem, TableAction } from '#/components/table/table-action';

  import UseCaseDetailForm from './form.vue';
  import { searchFormSchemas, tableColumns } from './schemas';

  defineOptions({
    name: 'UseCaseDetailManagement',
  });

  // ---------------- form -----------------

  const [FormModal, formModalApi] = useVbenModal({
    connectedComponent: UseCaseDetailForm,
  });
  ...
  const gridOptions: VxeGridProps<UseCaseDetailInfo> = {
    checkboxConfig: {
      highlight: true,
    },
    toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getUseCaseDetailList({
  ...
  async function batchDelete(ids: any[]) {
    const result = await deleteUseCaseDetail({
      ids,
    });
    if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t('use_case_detail.useCaseDetail.addUseCaseDetail') }}
        </Button>
      </template>
    </Grid>
  </Page>
