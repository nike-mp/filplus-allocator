...
} from '#/api/storage_provider/storageProvider';
...
defineOptions({
  name: 'StorageProviderListDrawer',
});
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  onOpenChange(isOpen: boolean) {
    if (isOpen) {
      applicationId.value = isOpen ? drawerApi.getData()?.applicationId || {} : {};
    }
  },
});
...
<template>
  <Drawer :title="$t('application.application.DetailStorageProvider')">
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t('storage_provider.storageProvider.addStorageProvider') }}
        </Button>
      </template>
    </Grid>
  </Drawer>
