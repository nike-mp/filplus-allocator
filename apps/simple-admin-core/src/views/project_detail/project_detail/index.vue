<script lang="ts" setup>
  ...
  import { deleteProjectDetail, getProjectDetailList } from '#/api/project_detail/projectDetail';
  import { type ActionItem, TableAction } from '#/components/table/table-action';

  import ProjectDetailForm from './form.vue';
  ...
  const gridOptions: VxeGridProps<ProjectDetailInfo> = {
    checkboxConfig: {
      highlight: true,
    },
    toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getProjectDetailList({
  ...
  async function batchDelete(ids: any[]) {
    const result = await deleteProjectDetail({
      ids,
    });
    if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t('project_detail.projectDetail.addProjectDetail') }}
        </Button>
      </template>
    </Grid>
  </Page>
