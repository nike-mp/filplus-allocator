<script lang="ts" setup>
  ...
  import { deleteAllocationPlan, getAllocationPlanList } from '#/api/allocation_plan/allocationPlan';
  import { type ActionItem, TableAction } from '#/components/table/table-action';

  import AllocationPlanForm from './form.vue';
  import { searchFormSchemas, tableColumns } from './schemas';

  defineOptions({
    name: 'AllocationPlanManagement',
  });
  ...
  const gridOptions: VxeGridProps<AllocationPlanInfo> = {
    checkboxConfig: {
      highlight: true,
    },
    toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getAllocationPlanList({
  ...
  async function batchDelete(ids: any[]) {
    const result = await deleteAllocationPlan({
      ids,
    });
    if (result.code === 0) {
    ...
    <Grid>
      ...
      <template #toolbar-tools>
        <Button type="primary" @click="openFormModal">
          {{ $t('allocation_plan.allocationPlan.addAllocationPlan') }}
        </Button>
      </template>
    </Grid>
  </Page>
